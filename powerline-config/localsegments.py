...
def get_git_status():
    ...
    for line in output.split('\n'):
        ...
        if line.find('HEAD detached at') >= 0:
            detached_head = True
            detatached_sha = re.findall('HEAD detached at ([^ ]+)', line)[0]
        if line.find('On branch') >= 0:
            current_branch = re.findall('On branch ([^ ]+)', line)[0]
    return has_pending_commits, has_untracked_files, origin_position, detached_head, detatached_sha, current_branch


@requires_segment_info
def branchdeluxe(pl, segment_info):
    ...
    has_pending_commits, has_untracked_files, origin_position, detached_head, detatached_sha, current_branch = get_git_status()

    if len(current_branch) > 0:
        branch = current_branch
    elif detached_head:
        branch = detatached_sha
        branch = '((' + branch + '))'
    ...
    scol.insert(0, 'branchdeluxe_dirty' if has_pending_commits else
                'branchdeluxe_clean')
...
def cwd(pl, segment_info, dir_shorten_len=None, dir_limit_depth=None, use_path_separator=False):
        ...
        for part in cwd:
                ...
                ret.append({
                        'contents': part,
                        'divider_highlight_groups': 'cwd:divider',
...
def mode(pl):
    ...
    if vimode.startswith("insert"):
        ret[0]['highlight_groups'] = 'mode_insert'
    elif vimode.startswith("command"):
        ret[0]['highlight_groups'] = 'mode_command'
...
def node(pl):
    version = subprocess.Popen('node --version',
...
def ruby(pl):
    version = subprocess.Popen("ruby --version | awk -F' ' '{print $2}'",
                               stdout=subprocess.PIPE,
                               stderr=subprocess.PIPE,
                               shell=True).stdout.read().strip()
